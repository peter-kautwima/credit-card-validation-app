...
import { validateLength, validationNumberRange, validationNumbersOnly } from './validation';
...
describe('validation', () => {
  ...
  it('validationNumberRange', () => {
    ...
    assert.equal(validationNumberRange(-1, 1, 12), 'Must be a number between 1 and 12');
    assert.equal(validationNumberRange(13, 1, 12), 'Must be a number between 1 and 12');
  });
